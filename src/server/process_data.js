const GeoJsonGeometriesLookup = require("../client/node_modules/geojson-geometries-lookup");
const fs = require("fs");
...
async function addToDB(dataset, named_map, entry_map) {
  ...
  for (const [index, month] of dataset.entries()) {
    ...
    let sql =
      "INSERT INTO `cld_mean` (location, cld, month, year, indexval) VALUES ?";
...
async function createTable() {
  ...
  var sql =
    "CREATE TABLE cld_mean (id INT AUTO_INCREMENT PRIMARY KEY, location VARCHAR(48),cld FLOAT(24),month CHAR(3), year SMALLINT(4), indexval INT(10))";
